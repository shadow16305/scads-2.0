...
const TokenomicsContent: React.FC<TokenomicsContentProps> = ({
  ...
  return (
    <div className="my-6 flex flex-col items-center gap-y-2 text-white">
      <p className="lg:max-w-[900px] xl:max-w-[1031px]">{paragraph}</p>
      {list && (
        <ul className="flex flex-col gap-y-2 lg:max-w-[900px] xl:max-w-[1031px]">
          {list.map((item, index) => (
            <li key={index}>
