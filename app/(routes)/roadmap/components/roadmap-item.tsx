...
const RoadmapItem: React.FC<RoadmapItemProps> = ({ date, l1, l2, l3 }) => {
  return (
    <div className="group relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse">
      <div className="bg-custom-color flex size-10 shrink-0 items-center justify-center rounded-full shadow md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2" />
      <motion.div
        initial={{ opacity: 0 }}
      ...
      >
        <div className="mb-1 flex items-center justify-between space-x-2">
          <time className="dark:text-custom-color font-semibold text-black">
            {date}
          </time>
        </div>
